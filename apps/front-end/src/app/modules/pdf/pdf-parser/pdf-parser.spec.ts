...
describe('PDF Parser', () => {
  describe('utils', () => {
    ...
    it('checkIfFirstLetterOfStringIsCaptial', () => {
      const checkIfFirstLetterOfStringIsCaptial =
        pdfParserObj['checkIfFirstLetterOfStringIsCaptial'];
    ...
    it('verifyIfPlayerTypeIsPresent', () => {
      const verifyIfPlayerTypeIsPresent =
        pdfParserObj['verifyIfPlayerTypeIsPresent'];
    ...
    it('verifyIfCardNumberIsPresent', () => {
      const verifyIfCardNumberIsPresent =
        pdfParserObj['verifyIfCardNumberIsPresent'];
    ...
    it('verifyIfHeaderTextIsPresent', () => {
      expect(
        pdfParserObj['verifyIfHeaderTextIsPresent'](
          'ROOKIE ARRIVAL AUTOGRAPH PATCH INSCRIPTION'
        )
      ).toBeTruthy();
      expect(
        pdfParserObj['verifyIfHeaderTextIsPresent'](
          'ULTIMATE STAGE CHROME CARDS AUTOGRAPH VARIATION'
        )
      ).toBeTruthy();
      expect(
        pdfParserObj['verifyIfHeaderTextIsPresent'](
      ...
      expect(
        pdfParserObj['verifyIfHeaderTextIsPresent']('COVER STARS')
      ).toBeTruthy();
      expect(
        pdfParserObj['verifyIfHeaderTextIsPresent']('BEST 10')
      ).toBeFalsy();
    });
    ...
    it('checkIfCardHasNoCardNumberButAllOthersAreSet', () => {
      ...
      expect(
        pdfParserObj['checkIfCardHasNoCardNumberButAllOthersAreSet'](failCheck)
      ).toBeFalsy();
    });
    it('checkIfCardHasAtleastCardNumAndSomeOthers', () => {
      ...
      expect(
        pdfParserObj['checkIfCardHasAtleastCardNumAndSomeOthers'](nonSet)
      ).toBeFalsy();
    });
  });
});
